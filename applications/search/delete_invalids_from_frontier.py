...
from datamodel.search.JasonhtTychua_datamodel import JasonhtTychuaLink
...
@Deleter(JasonhtTychuaLink)
@Getter(JasonhtTychuaLink)
class DeleteFrontierFrame(IApplication):
    ...
    def update(self):
        ...
        ls = self.frame.get(JasonhtTychuaLink)
        print "Found ", len(ls), " links."
        count = 0
        for l in ls:
            if not is_valid(l.full_url):
                self.frame.delete(JasonhtTychuaLink, l)
                count += 1
...
class Simulation(object):
    ...
    def __init__(self, address, port):
        ...
        objectless_connector = ObjectlessSpacetimeConnection(
            "ResetFrontier_JasonhtTychua".format(CrawlerFrame.app_id),
